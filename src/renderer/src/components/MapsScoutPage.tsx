...
// Open Telegram to send message to a business (opens in system default browser)
function openTelegram(lead: Lead): void {
  if (!lead.phone) return

  // Clean phone number - remove all non-numeric characters
  let cleanNumber = lead.phone.replace(/[^\d+]/g, '')
  // Ensure number starts with + for Telegram format
  if (!cleanNumber.startsWith('+')) {
    cleanNumber = '+' + cleanNumber
  }

  // Telegram deep link format: https://t.me/+<number>
  // This opens Telegram Desktop if installed, otherwise Telegram Web
  // Note: Telegram will show "User not found" if the number isn't on Telegram
  const tgUrl = `https://t.me/${cleanNumber}`
            ...
            {leads.map((lead, index) => (
              <div key={`${lead.id}-${index}`} className={`scout-lead ${lead.score}`}>
                ...
                <div className="lead-actions">
                  ...
                  {/* Send WhatsApp message button - show only if verified to have WhatsApp */}
                  {lead.hasWhatsApp === true && (
                    <button
                      className="lead-action whatsapp-send"
                  ...
                  {/* Try Telegram button - show for all leads with phone numbers */}
                  {lead.phone && (
                    <button
                      className="lead-action telegram-send"
                      title="Try Telegram"
